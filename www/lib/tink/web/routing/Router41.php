...
use \tink\querystring\Parser29 as QuerystringParser29;
use \tink\_Stringly\Stringly_Impl_;
use \tink\json\Parser29;
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:103: lines 103-142
					return new SyncFuture(new LazyConst((new Parser29())->tryParse($b->toString())));
				});
				break;
			...
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:103: lines 103-142
					return new SyncFuture(new LazyConst((new QuerystringParser29())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:103: lines 103-142
				$tmp = Promise_Impl_::ofOutcome(Outcome::Failure(new TypedError(406, "Cannot process Content-Type " . ($_g1??'null'), new HxAnon([
					...
					"lineNumber" => 611,
					"className" => "tink.web.routing.Router41",
					"methodName" => "files",
				]))));
				break;
		}
...
Boot::registerClass(Router41::class, 'tink.web.routing.Router41');
