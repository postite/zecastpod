...
use \tink\querystring\Parser35 as QuerystringParser35;
use \tink\core\Outcome;
use \tink\json\Parser35;
use \php\Boot;
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:108: lines 108-147
					return new SyncFuture(new LazyConst((new Parser35())->tryParse($b->toString())));
				});
				break;
			...
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:108: lines 108-147
					return new SyncFuture(new LazyConst((new QuerystringParser35())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:108: lines 108-147
				$tmp = Promise_Impl_::ofOutcome(Outcome::Failure(new TypedError(406, "Cannot process Content-Type " . ($_g1??'null'), new HxAnon([
					...
					"lineNumber" => 611,
					"className" => "tink.web.routing.Router48",
					"methodName" => "files",
				]))));
				break;
		}
...
Boot::registerClass(Router48::class, 'tink.web.routing.Router48');
