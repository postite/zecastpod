...
use \tink\json\Parser51;
use \tink\core\_Promise\Next_Impl_;
use \tink\querystring\Parser51 as QuerystringParser51;
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:113: lines 113-142
					return new SyncFuture(new LazyConst((new Parser51())->tryParse($b->toString())));
				});
				break;
			...
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:113: lines 113-142
					return new SyncFuture(new LazyConst((new QuerystringParser51())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:113: lines 113-142
				$tmp = Promise_Impl_::ofOutcome(Outcome::Failure(new TypedError(406, "Cannot process Content-Type " . ($_g1??'null'), new HxAnon([
					...
					"lineNumber" => 611,
					"className" => "tink.web.routing.Router68",
					"methodName" => "files",
				]))));
				break;
		}
...
Boot::registerClass(Router68::class, 'tink.web.routing.Router68');
