...
use \tink\querystring\Parser20 as QuerystringParser20;
use \tink\core\Outcome;
use \php\Boot;
use \tink\json\Parser20;
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:96: lines 96-135
					return new SyncFuture(new LazyConst((new Parser20())->tryParse($b->toString())));
				});
				break;
			...
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:96: lines 96-135
					return new SyncFuture(new LazyConst((new QuerystringParser20())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:96: lines 96-135
				$tmp = Promise_Impl_::ofOutcome(Outcome::Failure(new TypedError(406, "Cannot process Content-Type " . ($_g1??'null'), new HxAnon([
					...
					"lineNumber" => 611,
					"className" => "tink.web.routing.Router28",
					"methodName" => "files",
				]))));
				break;
		}
...
Boot::registerClass(Router28::class, 'tink.web.routing.Router28');
