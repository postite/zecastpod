...
use \tink\querystring\Parser25 as QuerystringParser25;
use \tink\core\_Promise\Next_Impl_;
use \tink\json\Parser25;
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:103: lines 103-142
					return new SyncFuture(new LazyConst((new Parser25())->tryParse($b->toString())));
				});
				break;
			...
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:103: lines 103-142
					return new SyncFuture(new LazyConst((new QuerystringParser25())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:103: lines 103-142
				$tmp = Promise_Impl_::ofOutcome(Outcome::Failure(new TypedError(406, "Cannot process Content-Type " . ($_g1??'null'), new HxAnon([
					...
					"lineNumber" => 611,
					"className" => "tink.web.routing.Router37",
					"methodName" => "files",
				]))));
				break;
		}
...
Boot::registerClass(Router37::class, 'tink.web.routing.Router37');
