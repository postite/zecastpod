...
use \tink\json\Parser8;
use \tink\json\Parser9;
use \tink\core\Outcome;
use \tink\querystring\Parser9 as QuerystringParser9;
use \php\Boot;
...
use \tink\querystring\Parser8 as QuerystringParser8;
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:118: lines 118-153
					return new SyncFuture(new LazyConst((new Parser8())->tryParse($b->toString())));
				});
				break;
			...
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:118: lines 118-153
					return new SyncFuture(new LazyConst((new QuerystringParser8())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:118: lines 118-153
				$tmp = Promise_Impl_::ofOutcome(Outcome::Failure(new TypedError(406, "Cannot process Content-Type " . ($_g1??'null'), new HxAnon([
					...
					"lineNumber" => 611,
					"className" => "tink.web.routing.Router10",
					"methodName" => "files",
				]))));
				break;
		}
	...
	public function filesUpdate ($ctx) {
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:170: lines 170-182
					return new SyncFuture(new LazyConst((new Parser9())->tryParse($b->toString())));
				});
				break;
			case "application/x-www-form-urlencoded":
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:170: lines 170-182
					return new SyncFuture(new LazyConst((new QuerystringParser9())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:170: lines 170-182
...
Boot::registerClass(Router10::class, 'tink.web.routing.Router10');
