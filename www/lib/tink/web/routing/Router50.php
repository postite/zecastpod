...
use \tink\querystring\Parser37 as QuerystringParser37;
use \tink\core\_Promise\Next_Impl_;
use \tink\json\Parser37;
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:108: lines 108-147
					return new SyncFuture(new LazyConst((new Parser37())->tryParse($b->toString())));
				});
				break;
			...
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:108: lines 108-147
					return new SyncFuture(new LazyConst((new QuerystringParser37())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:108: lines 108-147
				$tmp = Promise_Impl_::ofOutcome(Outcome::Failure(new TypedError(406, "Cannot process Content-Type " . ($_g1??'null'), new HxAnon([
					...
					"lineNumber" => 611,
					"className" => "tink.web.routing.Router50",
					"methodName" => "files",
				]))));
				break;
		}
...
Boot::registerClass(Router50::class, 'tink.web.routing.Router50');
