...
use \tink\json\Parser47;
use \tink\querystring\Parser47 as QuerystringParser47;
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:112: lines 112-141
					return new SyncFuture(new LazyConst((new Parser47())->tryParse($b->toString())));
				});
				break;
			...
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:112: lines 112-141
					return new SyncFuture(new LazyConst((new QuerystringParser47())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:112: lines 112-141
				$tmp = Promise_Impl_::ofOutcome(Outcome::Failure(new TypedError(406, "Cannot process Content-Type " . ($_g1??'null'), new HxAnon([
					...
					"lineNumber" => 611,
					"className" => "tink.web.routing.Router60",
					"methodName" => "files",
				]))));
				break;
		}
...
Boot::registerClass(Router60::class, 'tink.web.routing.Router60');
