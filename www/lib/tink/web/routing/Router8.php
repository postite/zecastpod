...
use \tink\querystring\Parser7 as QuerystringParser7;
use \tink\core\Outcome;
use \tink\json\Parser7;
use \php\Boot;
		...
		switch ($_g1) {
			...
			default:
				...
				$tmp = Promise_Impl_::ofOutcome(Outcome::Failure(new TypedError(406, "Cannot process Content-Type " . ($_g1??'null'), new HxAnon([
					...
					"lineNumber" => 611,
					"className" => "tink.web.routing.Router8",
					"methodName" => "files",
				]))));
				break;
		}
	...
	public function filesUpdate ($ctx) {
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:170: lines 170-182
					return new SyncFuture(new LazyConst((new Parser7())->tryParse($b->toString())));
				});
				break;
			case "application/x-www-form-urlencoded":
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:170: lines 170-182
					return new SyncFuture(new LazyConst((new QuerystringParser7())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:170: lines 170-182
								...
								switch ($_g1) {
									...
									case true:
										...
										if ($_g === false) {
											...
											return Promise_Impl_::ofSpecific($this->up($ctx, $_g3, null));
										} else {
											#src/Server.hx:29: characters 16-44
...
Boot::registerClass(Router8::class, 'tink.web.routing.Router8');
