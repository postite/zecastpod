...
use \tink\querystring\Parser44 as QuerystringParser44;
use \tink\json\Parser44;
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:109: lines 109-148
					return new SyncFuture(new LazyConst((new Parser44())->tryParse($b->toString())));
				});
				break;
			...
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:109: lines 109-148
					return new SyncFuture(new LazyConst((new QuerystringParser44())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:109: lines 109-148
				$tmp = Promise_Impl_::ofOutcome(Outcome::Failure(new TypedError(406, "Cannot process Content-Type " . ($_g1??'null'), new HxAnon([
					...
					"lineNumber" => 611,
					"className" => "tink.web.routing.Router57",
					"methodName" => "files",
				]))));
				break;
		}
...
Boot::registerClass(Router57::class, 'tink.web.routing.Router57');
