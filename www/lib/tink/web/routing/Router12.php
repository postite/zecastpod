...
use \tink\querystring\Parser13 as QuerystringParser13;
use \tink\querystring\Parser12 as QuerystringParser12;
use \tink\_Stringly\Stringly_Impl_;
use \tink\json\Parser13;
use \tink\core\_Promise\Next_Impl_;
use \tink\json\Parser12;
...
class Router12 {
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:118: lines 118-153
					return new SyncFuture(new LazyConst((new Parser12())->tryParse($b->toString())));
				});
				break;
			...
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:118: lines 118-153
					return new SyncFuture(new LazyConst((new QuerystringParser12())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:118: lines 118-153
				$tmp = Promise_Impl_::ofOutcome(Outcome::Failure(new TypedError(406, "Cannot process Content-Type " . ($_g1??'null'), new HxAnon([
					...
					"lineNumber" => 611,
					"className" => "tink.web.routing.Router12",
					"methodName" => "files",
				]))));
				break;
		}
	...
	public function filesRec ($ctx) {
		...
		return Promise_Impl_::next($tmp, function ($body)  use (&$_gthis) {
			#src/Server.hx:156: lines 156-172
			return Promise_Impl_::next($_gthis->target->filesRec($body)->map(new HxClosure(Outcome::class, 'Success'))->gather(), Next_Impl_::ofSafeSync(function ($v1) {
				#src/Server.hx:156: lines 156-172
				return Response_Impl_::ofHtml($v1);
			}));
		});
	...
	public function filesUpdate ($ctx) {
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:175: lines 175-194
					return new SyncFuture(new LazyConst((new Parser13())->tryParse($b->toString())));
				});
				break;
			case "application/x-www-form-urlencoded":
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:175: lines 175-194
					return new SyncFuture(new LazyConst((new QuerystringParser13())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:175: lines 175-194
				...
				switch ($_g6) {
					...
					case "filesUpdate":
						...
						if ($_g3 === true) {
							#/Users/ut/haxe/haxe_libraries/tink_web/0.1.4/github/7c93ab66f476c97e62510c653bb4e58e918e1471/src/tink/web/macros/Routing.hx:143: characters 22-31
							if ($_g2 === false) {
								#src/Server.hx:174: characters 3-8
...
Boot::registerClass(Router12::class, 'tink.web.routing.Router12');
