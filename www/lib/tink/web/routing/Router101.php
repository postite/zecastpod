...
use \tink\querystring\Parser73 as QuerystringParser73;
use \php\Boot;
...
use \tink\json\Parser74;
use \tink\json\Parser73;
use \tink\querystring\Parser74 as QuerystringParser74;
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:114: lines 114-149
					return new SyncFuture(new LazyConst((new Parser73())->tryParse($b->toString())));
				});
				break;
			...
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:114: lines 114-149
					return new SyncFuture(new LazyConst((new QuerystringParser73())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:114: lines 114-149
				$tmp = Promise_Impl_::ofOutcome(Outcome::Failure(new TypedError(406, "Cannot process Content-Type " . ($_g1??'null'), new HxAnon([
					...
					"lineNumber" => 611,
					"className" => "tink.web.routing.Router101",
					"methodName" => "files",
				]))));
				break;
		}
	...
	public function filesUpdate ($ctx) {
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:152: lines 152-164
					return new SyncFuture(new LazyConst((new Parser74())->tryParse($b->toString())));
				});
				break;
			case "application/x-www-form-urlencoded":
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:152: lines 152-164
					return new SyncFuture(new LazyConst((new QuerystringParser74())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:152: lines 152-164
...
Boot::registerClass(Router101::class, 'tink.web.routing.Router101');
