...
use \tink\json\Parser33;
use \tink\_Stringly\Stringly_Impl_;
use \tink\querystring\Parser33 as QuerystringParser33;
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:108: lines 108-147
					return new SyncFuture(new LazyConst((new Parser33())->tryParse($b->toString())));
				});
				break;
			...
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:108: lines 108-147
					return new SyncFuture(new LazyConst((new QuerystringParser33())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:108: lines 108-147
				$tmp = Promise_Impl_::ofOutcome(Outcome::Failure(new TypedError(406, "Cannot process Content-Type " . ($_g1??'null'), new HxAnon([
					...
					"lineNumber" => 611,
					"className" => "tink.web.routing.Router46",
					"methodName" => "files",
				]))));
				break;
		}
...
Boot::registerClass(Router46::class, 'tink.web.routing.Router46');
