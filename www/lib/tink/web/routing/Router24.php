...
use \tink\querystring\Parser16 as QuerystringParser16;
use \tink\json\Parser16;
		...
		switch ($_g1) {
			case "application/json":
				...
				$tmp = Promise_Impl_::next($ctx->allRaw(), function ($b) {
					#src/Server.hx:96: lines 96-135
					return new SyncFuture(new LazyConst((new Parser16())->tryParse($b->toString())));
				});
				break;
			...
			case "multipart/form-data":
				...
				$tmp = Promise_Impl_::next($ctx->parse(), function ($pairs) {
					#src/Server.hx:96: lines 96-135
					return new SyncFuture(new LazyConst((new QuerystringParser16())->tryParse(Pairs_Impl_::ofIterable($pairs))));
				});
				break;
			default:
				#src/Server.hx:96: lines 96-135
				$tmp = Promise_Impl_::ofOutcome(Outcome::Failure(new TypedError(406, "Cannot process Content-Type " . ($_g1??'null'), new HxAnon([
					...
					"lineNumber" => 611,
					"className" => "tink.web.routing.Router24",
					"methodName" => "files",
				]))));
				break;
		}
...
Boot::registerClass(Router24::class, 'tink.web.routing.Router24');
